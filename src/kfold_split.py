...
samples = json.load(open("../data/reddit/reddit_data_with_sarcasm.json", encoding="utf-8"))
...
for not_test_index, test_index in kf.split(X=new_samples, y=subreddits, groups=subreddits):
    ...
    if not os.path.exists("../data/reddit/split1/"):
        os.makedirs("../data/reddit/split1/")
    if not os.path.exists(os.path.join("../data/reddit/split1/", str(i))):
        os.makedirs(os.path.join("../data/reddit/split1/", str(i)))
    with open(os.path.join("../data/reddit/split1/"+str(i), "train_ids.json"), "w", encoding="utf-8") as output_file:
        json.dump(not_test_samples, output_file, indent=4)
    with open(os.path.join("../data/reddit/split1/"+str(i), "dev_ids.json"), "w", encoding="utf-8") as output_file:
        json.dump(test_samples, output_file, indent=4)
    with open(os.path.join("../data/reddit/split1/"+str(i), "test_ids.json"), "w", encoding="utf-8") as output_file:
        json.dump(test_samples, output_file, indent=4)
    i += 1