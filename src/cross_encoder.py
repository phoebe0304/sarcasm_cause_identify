...
class BertClause(BertPreTrainedModel):
    ...
    def __init__(self, config,co_hidden_size=768, co_intermediate_size=1536, clause_max_len=None, max_len=None):
        ...
        # Set missing config attributes with default values为配置参数设置默认值
        if not hasattr(config, 'attention_head'):
            config.attention_head = getattr(config, 'num_attention_heads', 12)
        if not hasattr(config, 'other_hidden_dropout_prob'):
            config.other_hidden_dropout_prob = getattr(config, 'hidden_dropout_prob', 0.1)
        if not hasattr(config, 'src_dropout_prob'):
            config.src_dropout_prob = getattr(config, 'hidden_dropout_prob', 0.1)
        if not hasattr(config, 'num_connection_layers'):
            config.num_connection_layers = 1
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, input_mask=None,
        ...
        if not return_dict:
            # 不使用 return_dict 时，直接返回 (loss, logits)
            if clause_loss is not None:
                return clause_loss, clause_logits
            else:
    ...
    def sents_index_select(self, sents_output, idx):
        """
        sents_output: [B, num_sents, clause_max_len, hidden]
        idx: [B, idx_num]
        """
        B, num_sents, clause_max_len, hidden = sents_output.shape
        idx_clamped = idx.clone()

        # ---- 关键修复：强制 idx 不越界 ----
        idx_clamped = torch.clamp(idx_clamped, min=0, max=num_sents - 1)

        # reshape
        dummy = idx_clamped.unsqueeze(-1).unsqueeze(-1).expand(B, idx.shape[1], clause_max_len, hidden)

        # gather
        selected = sents_output.gather(1, dummy)

        return selected

    def att_mask_index_select(self, sents_mask, idxs):
        """
        sents_mask: [batch_size, max_sents_num, clause_max_len]
        idxs: [batch_size, idx_num] (may contain -1 for padding)
        returns: [batch_size, idx_num, clause_max_len]
        """
        B, num_sents, clause_max_len = sents_mask.shape
        idx_clamped = torch.clamp(idxs, min=0, max=num_sents - 1)  # 修复越界索引
        dummy = idx_clamped.unsqueeze(2).expand(B, idxs.size(1), clause_max_len)

        select_mask = sents_mask.gather(1, dummy)  # [B, idx_num, clause_max_len]
