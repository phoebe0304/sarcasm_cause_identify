...
def get_bert_base_loader(config, prefix):
    ...
    with open(os.path.join(config.data_path, prefix + ".json"), "r") as f:
        data = json.load(f)

    for item in data:
        text = " ".join(item["clauses"])
        label = item["label"]
        texts.append(text)
        labels.append(label)
    ...
    return torch.utils.data.DataLoader(
        dataset,
        batch_size=config.batch_size,
        shuffle=(prefix == config.train_prefix)
    )
